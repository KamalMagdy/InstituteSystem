class AssignmentstaffstudentsController < InheritedResources::Base
  def index
    ...
    @studentids = ActiveRecord::Base.connection.execute("select student_id from assignmentstaffstudents")
    puts @studentids
    for studentid in @studentids
      @student_name.push(ActiveRecord::Base.connection.execute("select name from students where id=#{studentid['student_id']}"))
      # puts @student_name
    end
  end
  def new  
